...
var minColorValue = 0.019;      // Values below this appear as white (underflow)
var maxColorValue = 2.047;     // Values above this appear as black (overflow)
...
function evaluatePixel(sample) {    let a = 0.496;

    let index = (Math.abs(((sample.B05 - sample.B03) / 150.0 * sample.B04 + sample.B04 + sample.B03 - (a * sample.B03))) / Math.pow((Math.pow(a, 2.0) + 1.0), 0.5)) * (sample.B05 / sample.B04);
