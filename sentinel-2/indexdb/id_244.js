...
var minColorValue = -0.625;      // Values below this appear as white (underflow)
var maxColorValue = 1.18;     // Values above this appear as black (overflow)
...
function evaluatePixel(sample) {    let index = (((sample.B04 - sample.B03) / (sample.B04 + sample.B03)) + 0.5) / Math.abs(((sample.B04 - sample.B03) / (sample.B04 + sample.B03)) + 0.5) * Math.sqrt(Math.abs((((sample.B04 - sample.B03) / (sample.B04 + sample.B03))) + 0.5));
    let zero = 0.0;
