...
function setup() {
    return {
        input: ["B02", "B03", "B08", "SCL", "dataMask"],
        output: [
            { id: "default", bands: 4 },
...
var minColorValue = -0.938;      // Values below this appear as white (underflow)
var maxColorValue = 0.529;     // Values above this appear as black (overflow)
...
function evaluatePixel(sample) {    let index = (sample.B08 - (sample.B03 + sample.B02)) / (sample.B08 + (sample.B03 + sample.B02));
    let zero = 0.0;
