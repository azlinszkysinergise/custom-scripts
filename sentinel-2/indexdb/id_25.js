...
var minColorValue = -0.433;      // Values below this appear as white (underflow)
var maxColorValue = 0.808;     // Values above this appear as black (overflow)
...
function evaluatePixel(sample) {    let index = ((2.0 * (Math.pow(sample.B08, 2.0) - Math.pow(sample.B04, 2.0)) + 1.5 * sample.B08 + 0.5 * sample.B04) / (sample.B08 + sample.B04 + 0.5) * (1.0 - 0.25 * (2.0 * (Math.pow(sample.B08, 2.0) - Math.pow(sample.B04, 2.0)) + 1.5 * sample.B08 + 0.5 * sample.B04) / (sample.B08 + sample.B04 + 0.5)) - (sample.B04 - 0.125) / (1.0 - sample.B04));
    let zero = 0.0;
