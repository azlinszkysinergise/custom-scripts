...
var minColorValue = -0.598;      // Values below this appear as white (underflow)
var maxColorValue = 1.032;     // Values above this appear as black (overflow)
...
function evaluatePixel(sample) {    let index = 1.5 * (2.5 * (sample.B08 - sample.B04) - 1.3 * (sample.B08 - sample.B03)) / Math.sqrt(Math.pow((2.0 * sample.B08 + 1.0), 2.0) - (6.0 * sample.B08 - 5.0 * Math.sqrt(sample.B04)) - 0.5);
    let zero = 0.0;
